...
def comcat(
    ...
    # recode batch labels to 0-based integers
    ...
    grand_mean = np.mean(XZ_no_preserve @ beta_hat[:n_batch + n_Z, :], axis=0)  # (n_valid,)
    ...
    delta_hat_masked = np.zeros((n_batch + n_Z, Ym.shape[0]), dtype=np.float32)
